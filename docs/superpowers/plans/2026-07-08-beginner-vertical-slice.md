...
- [x] **Step 4: Приёмка v1 по чек-листу спеки** — прогнано 2026-07-08 (e2e Playwright по прод-URL, 29/29):

- [x] новый профиль не попадает на Мостик до пролога/skip — e2e PASS на прод-URL;
- [x] пролог: каждая сцена переживает перезагрузку — e2e PASS (F5 на сцене 4 продолжает с неё);
- [x] калибровка без сети и без LLM — e2e PASS (context.setOffline на все 8 заданий);
- [x] три fixture-профиля → ожидаемые результаты — юнит-тесты scoring (novice/mixed/strong-code) зелёные; override «пройти всё подряд» покрыт тестом recommendMission;
- [x] обе главы открываются и CODE-01 проходима на 390×844 И 360×800 без горизонтального скролла — e2e PASS оба viewport (остальные миссии — та же механика тех же 5 рендереров);
- [x] KORA: фолбэк «Бортовая справка · KORA офлайн» без ошибок в UI — e2e PASS; живой ответ — после ручного деплоя воркера владельцем (README);
- [ ] воркер отвергает запросы без секрета/сверх бюджета — curl из README **после деплоя воркера владельцем** (guard-логика покрыта юнит-тестами 401/403/405/413/429);
- [x] чистая установка восстанавливается из экспортированного файла — e2e PASS (очистка localStorage → «восстановить из файла» на первой сцене → Мостик с прогрессом);
- [ ] PWA ставится на Android с Pages — **проверка владельцем на телефоне**;
- [x] CI зелёный: typecheck + tests (53) + validate:content --strict + build + smoke — run 28904606071 success, сайт живой.
